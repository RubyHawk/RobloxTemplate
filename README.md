...
To deliver tested gameplay code, the selected independent UI pack, and authored portal assets into that same permanent RNG Defender place, use **`8_RNG_DEFENDER.cmd`**. It validates the exact configured universe/place, updates the private Stagewright plugin, verifies that exactly one official Rojo Studio plugin is installed, serves only the safe `$ignoreUnknownInstances` patch, and refuses to connect to another place. Team Create remains the sole owner of every island, bridge, upgrade area, and other map-scene object; the safe patch owns no map scenery. The RNG recipe stores only the logical layout center, and Stagewright derives six 32×27 playable grid footprints from the configured slot-1 offset and yaw in 60-degree rotations. One admin `GamePlatform` remains the single authoring source rather than being cloned into those slots. The legacy **`SERVE_TOWER_DEFENSE.cmd`** shortcut delegates to this same guarded launcher. Connect Rojo in Studio, verify in Play, then use **File > Publish to Roblox** on that same experience. Never publish the standalone `build/RNGDefenderSafePatch.rbxlx`: it is a structural validation build and intentionally omits unknown Team Create world data.
