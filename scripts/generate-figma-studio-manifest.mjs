...
const DEFAULT_RUNTIME = "build/FigmaUiBridgeManifest.json";
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!["workspace", "delivery", "output", "runtime", "patch"].includes(key)) {
...
async function build(options) {
  ...
  await Promise.all([
    ...
    mkdir(path.dirname(deliveryPath), { recursive: true }),
  ]);
  ...
  const runtimeText = JSON.stringify(manifest);
  ...
  if (options.manifestOnly) {
    ...
    await Promise.all([
      ...
      writeFile(runtimePath, runtimeText, "utf8"),
    ]);
    return delivery;
  }

  if (options.check) {
    const [currentLua, currentDelivery, currentRuntime] = await Promise.all([
      readFile(outputPath, "utf8"),
      readFile(deliveryPath, "utf8"),
      readFile(runtimePath, "utf8"),
    ]);
    if (currentLua !== lua || currentDelivery !== deliveryText || currentRuntime !== runtimeText) {
