...
- Put game-specific values in `TemplateConfig`; never hardcode product, group, audio, notification, or experience IDs in services.
- Reuse the two permanent Roblox experiences in `experiences.config.json` (`template` = mock UI workbench, `playerTest` = live player testing); never create a new Roblox experience per build or test.
